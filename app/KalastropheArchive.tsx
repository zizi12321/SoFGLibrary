  ...
  },
  {
    ...
    "powers": [
      ...
      },
      {
        ...
        "limit": "已通过 <CrossReference name=\"A Power to Assist\" /> 选择本分支（专长标签 Undead）。目标为拥有已渗透 <CrossReference name=\"Catacombs\" href=\"?page=points-of-interest#entry-location-sub-catacombs\" meta=\"兴趣点\" text=\"当地 Death 低于 200 时，每回合从每个相邻地点吸收最多 5 Death，并转入当地。200 是开始吸收前的检查值，多个邻地同时转入时可以越过该值。\n\n出现方式\n普通小型聚居地生成 Holy Site 后，另有 50% 概率配置。\n\n可出现地点\n小型人类聚居地。\n改建或覆灭后是否保留，还受对应流程限制。\" image=\"/locations/game/catacombs.png\" target=\"_blank\" /> 的人类类聚居地。"
      },
      {
        "name": "Defile",
        "seal": 2,
        ...
        "effect": "把 <CrossReference name=\"Holy Site\" href=\"?page=points-of-interest#entry-location-sub-cathedral\" meta=\"兴趣点\" text=\"未被亵渎时提供 Consecrate、Desecrate Holy Site 与 Cleanse Own Soul；仍可提供适用的 Blasphemous Rituals。\n被亵渎后每回合增加当地 Shadow 1%，最高 100%。Dark Empire 会使其进入亵渎状态；Ophanim 控制会解除该状态；亵渎的圣所在 Alliance 中每回合增加 2 Menace。\n\n出现方式\n小型人类聚居地的随机候选；附近已有 Holy Site 时，不加入新地点的候选池。\n\n可出现地点\n小型人类聚居地。\n改建或覆灭后是否保留，还受对应流程限制。\" image=\"/locations/game/cathedral.png\" target=\"_blank\" /> 的 Consecrate 行动替换为不可执行的 <CrossReference name=\"Shattered Beliefs\" />。",
      ...
      },
      {
        ...
        "limit": "已通过 <CrossReference name=\"A Power to Assist\" /> 选择本分支（专长标签 Deep Ones）。目标为 <CrossReference name=\"Deep One Sanctum\" href=\"?page=locations#entry-location-set-deeponesanctum\" meta=\"地点\" text=\"人类聚居地被 Deep Ones 接管后的陆地据点。初始 Shadow 为 100%，防御上限为 50；固定增加 Sanctum，并接收旧聚居地的兴趣点。\n\n出现方式\nDeep One Cult 的转化流程建立。旧兴趣点继续影响此处可执行的任务。\n\n可能配置的兴趣点\nSanctum。\n固定、随机与改建来源见各兴趣点。\n\n自身任务列表为空；当地修正、保留的兴趣点或特殊单位可能另提供行动。\" image=\"/locations/game/loc_evil_deepOneSanctum.png\" target=\"_blank\" />。"
      },
      {
        "name": "Fickle Hearts",
        "seal": 2,
  ...
  },
  {
    ...
    "powers": [
      ...
      },
      {
        ...
        "limit": "已通过 <CrossReference name=\"A Power to Interfere\" /> 选择本分支（性格标签 Religion）。目标必须是带 <CrossReference name=\"Desecrated Holy Site\" href=\"?page=points-of-interest#entry-location-sub-cathedral\" meta=\"兴趣点\" text=\"未被亵渎时提供 Consecrate、Desecrate Holy Site 与 Cleanse Own Soul；仍可提供适用的 Blasphemous Rituals。\n被亵渎后每回合增加当地 Shadow 1%，最高 100%。Dark Empire 会使其进入亵渎状态；Ophanim 控制会解除该状态；亵渎的圣所在 Alliance 中每回合增加 2 Menace。\n\n出现方式\n小型人类聚居地的随机候选；附近已有 Holy Site 时，不加入新地点的候选池。\n\n可出现地点\n小型人类聚居地。\n改建或覆灭后是否保留，还受对应流程限制。\" image=\"/locations/game/cathedral.png\" target=\"_blank\" /> 的小型人类聚居地。"
      }
    ]
  },
  {
    ...
    },
{
  ...
  "items": [
    ...
    {
      "name": "Deep One Cult",
      "text": "强度超过 100 后向地点提供 <CrossReference name=\"Shadow\" />；当 Madness 低于“Cult 强度 −100”时向当地提供 <CrossReference name=\"Madness\" />。强度达到 300 后使聚居地转为 <CrossReference name=\"Deep One Sanctum\" href=\"?page=locations#entry-location-set-deeponesanctum\" meta=\"地点\" text=\"人类聚居地被 Deep Ones 接管后的陆地据点。初始 Shadow 为 100%，防御上限为 50；固定增加 Sanctum，并接收旧聚居地的兴趣点。\n\n出现方式\nDeep One Cult 的转化流程建立。旧兴趣点继续影响此处可执行的任务。\n\n可能配置的兴趣点\nSanctum。\n固定、随机与改建来源见各兴趣点。\n\n自身任务列表为空；当地修正、保留的兴趣点或特殊单位可能另提供行动。\" image=\"/locations/game/loc_evil_deepOneSanctum.png\" target=\"_blank\" />；人类人口的 70% 经 Sanctum 转送至深海城市，统治者可变成自主 <CrossReference name=\"Deep One\" />。Security 修正为 −⌊（50+强度）÷100⌋。\nProfile 和 Menace 平时会继续增长；<CrossReference name=\"Parasitic Integration\" /> 可以清零两者并长期加速成熟。",
      "image": "deepOnes.png",
      "initialValue": "",
      "modifierChange": {
        "natural": "通常每回合强度 +1。",
    ...
    },
{
  ...
  "items": [
    ...
    },
    {
      ...
      "text": "把当地改为已渗透的 <CrossReference name=\"Orc Camp\" href=\"?page=locations#entry-location-set-orccamp\" meta=\"地点\" text=\"Orc 营地用专精区分普通 Camp、Fortress、Mage Camp、Menagerie、Empty Shipyard、Shipyard 与 Mines。这些是营地状态，不是附加兴趣点。\n普通营地可建设专精；Mage Camp 增加 Spelltwister 招募；Menagerie 增加特殊随从。Fortress、Mage Camp 能重建常备军，Mines 允许部落进入地下。\n随从招募：普通营地提供 Orc Warrior、Goblin；Mage Camp 另有 Spelltwister；Menagerie 另有 Goblin Infiltrator、Webspinner、Razor Rat、Giggler、Changeling。\n\n出现方式\n地图生成或 Orc 扩张建立。新营地加入 5 强度 Orcish Industry。基础防御上限为 50 + Orcish Defences / 2，每回合恢复 2 防御，不超过上限。\" image=\"/locations/game/loc_evil_orc.png\" target=\"_blank\" />，归执行者所属势力；原有设施保留。",
    ...
    "effects": [
      {
        ...
        "href": "?page=locations#entry-location-set-deeponesanctum",
        "meta": "地点",
        "text": "人类聚居地被 Deep Ones 接管后的陆地据点。初始 Shadow 为 100%，防御上限为 50；固定增加 Sanctum，并接收旧聚居地的兴趣点。\n\n出现方式\nDeep One Cult 的转化流程建立。旧兴趣点继续影响此处可执行的任务。\n\n可能配置的兴趣点\nSanctum。\n固定、随机与改建来源见各兴趣点。\n\n自身任务列表为空；当地修正、保留的兴趣点或特殊单位可能另提供行动。",
