...
type DetailItem = { name: string; text: string; image?: string; images?: string[]; seal?: number; meta?: string; statLine?: string; preferenceText?: string; id?: string; baseGame?: boolean };
...
const familyCurses: DetailItem[] = [
  ...
  { seal: 2, name: "The Howling", id: "base-the-howling", baseGame: true, text: "游戏本体家族诅咒。每回合检查家族成员，并把其最大 Sanity 压到不高于 4，使人物更容易陷入疯狂。Vile Curse 可以选择把它施加给目标家族。" },
  ...
  { seal: 2, name: "Wasting Souls", id: "base-wasting-souls", baseGame: true, text: "游戏本体家族诅咒。家族成员每回合损失 5 XP；Chosen One 不受影响。Vile Curse 的选择菜单中写作 Wasting Soul，实际添加的是这一诅咒。" },
  { seal: 2, name: "See Through Their Eyes", id: "base-see-through-their-eyes", baseGame: true, text: "游戏本体家族诅咒。为非玩家控制的家族成员赋予 Through their Eyes，使其统治或当前停留的聚居地 Security −2；成为玩家可控制 Agent 时该人物特质会被移除。Vile Curse 的选择菜单中写作 Through Their Eyes。" },
];
...
const minions: DetailItem[] = [
  { name: "Sellsword", baseGame: true, text: "本体随从。Beguiler 每 5 回合检查一次 Supplicant 的随从槽，并免费把一名 Sellsword 放进第一个空位。" },
];
...
function DetailGrid({ items, openEntries, onToggle, media = true }: { items: DetailItem[]; openEntries: Set<string>; onToggle: (id: string) => void; media?: boolean }) {
  return <div className="expandable-table">{items.map((item) => { const entryId = item.id ? `entry-${item.id}` : anchorFor(item.name); const rel = relations[item.id ?? item.name]; return <ExpandableRow key={entryId} entryId={entryId} name={item.name} seal={item.seal} image={media ? item.image ?? item.images?.[0] : undefined} baseGame={item.baseGame} open={openEntries.has(entryId)} onToggle={onToggle}>{item.meta && <div className="expanded-meta">{item.meta}</div>}{item.statLine && <div className="task-stat-line">{item.statLine}</div>}<p><RichText text={item.text} exclude={item.name} /></p>{item.preferenceText && <div className="preference-note"><b>执行倾向</b><p><RichText text={item.preferenceText} exclude={item.name} /></p></div>}<RelationGroup title="来源" items={rel?.sources} /><RelationGroup title="造成的效果" items={rel?.effects} /></ExpandableRow>; })}</div>;
}
