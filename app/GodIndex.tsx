...
export default function GodIndex({ onSelect }: { onSelect: (god: GodChoice) => void }) {
  ...
  return <main className="archive-index"><header className="index-header"><div className="index-brand"><CircleDot size={18} /><span>Shadows of Forbidden Gods</span></div><p>神祇资料库</p><h1>选择神祇</h1><div className="index-intro"><BookOpen size={18} /><span>基于游戏本体程序集、模组 DLL、事件定义与原始美术素材整理。每个页面均可查看封印、神力、Agent、单位、地点修正、任务与相关机制。</span></div></header><section className="god-index-group" aria-labelledby="base-gods-heading"><div className="index-group-heading"><p>游戏本体</p><h2 id="base-gods-heading">游戏本体</h2><span>9 位神祇</span></div><div className="god-index-grid">{baseGods.map(renderCard)}</div></section><section className="god-index-group" aria-labelledby="mod-gods-heading"><div className="index-group-heading"><p>Mod</p><h2 id="mod-gods-heading">Mod 神祇</h2><span>3 位神祇</span></div><div className="god-index-grid">{modGods.map(renderCard)}</div></section><footer className="index-footer">当前共收录 {gods.length} 位神祇</footer></main>;
}