...
export function DetailGrid({ config, items, openEntries, onToggle }: { config: ArchiveRecordConfig; items: DetailItem[]; openEntries: Set<string>; onToggle: (id: string) => void }) {
 return <div className="expandable-table">{items.map(item => { const id = item.id ? "entry-" + item.id : anchorFor(item.name); const relation = config.relations?.[item.id ?? item.name] ?? config.relations?.[item.name]; return <ExpandableRow key={id} config={config} entryId={id} name={item.name} seal={item.seal} image={item.image} baseGame={item.baseGame} open={openEntries.has(id)} onToggle={onToggle}>{item.tenetRange && <small className="tenet-range">等级范围：{item.tenetRange}</small>}{item.meta && <div className="expanded-meta">{item.meta}</div>}{item.location && <div className="task-location"><b>执行地点</b><RichText config={config} text={item.location} exclude={item.name} /></div>}{item.time && <div className="detail-stat-line"><b>执行时间：</b>{item.time}</div>}{item.initialValue && !/未找到/.test(item.initialValue) && <div className="detail-stat-line"><b>初始值：</b><RichText config={config} text={item.initialValue} exclude={item.name} /></div>}{item.stats && <div className="detail-stat-line"><b>基础属性：</b><RichText config={config} text={item.stats} exclude={item.name} /></div>}{item.abilities?.length ? <AbilityList config={config} abilities={item.abilities} /> : null}{item.statLine && <div className="task-stat-line">{item.statLine}</div>}{item.positiveTags && <div className="detail-stat-line"><b>正面标签：</b>{item.positiveTags}</div>}{item.negativeTags && <div className="detail-stat-line"><b>负面标签：</b>{item.negativeTags}</div>}{item.limit ? <><div className="expanded-section"><h4>具体效果</h4><p><RichText config={config} text={item.text} exclude={item.name} /></p></div><div className="expanded-section"><h4>释放限制</h4><p><RichText config={config} text={item.limit} exclude={item.name} /></p></div></> : item.text ? <p><RichText config={config} text={item.text} exclude={item.name} /></p> : null}{item.tenetLevels?.map(level => <p className="tenet-level" key={level.level}><b>{level.level}:</b> <RichText config={config} text={level.text} exclude={item.name} /></p>)}{item.eventOptions?.length ? <div className="event-options"><h4>事件选项</h4><ol>{item.eventOptions.map((option, index) => <li key={index}><h5>{option.name}</h5>{option.condition && <p className="option-condition"><b>条件：</b><RichText config={config} text={option.condition} /></p>}<p><RichText config={config} text={option.text} exclude={item.name} /></p></li>)}</ol></div> : null}{item.modifierChange && (item.modifierChange.natural || item.modifierChange.external) && <div className="modifier-change"><h4>变化方式</h4>{item.modifierChange.natural && <p><RichText config={config} text={item.modifierChange.natural} exclude={item.name} /></p>}{item.modifierChange.external && <p><RichText config={config} text={item.modifierChange.external} exclude={item.name} /></p>}</div>}<RelationGroup config={config} title="来源" items={relation?.sources} /><RelationGroup config={config} title="造成的效果" items={relation?.effects} /></ExpandableRow>; })}</div>;
}
...
function OverviewSection({ config }: { config: GodConfig }) {
 ...
 return <section id="loop" className="section overview-section">
  ...
  <div className="overview-layout"><article className="overview-main">
   ...
   <div><dt>初始 Agent 上限</dt><dd>{config.seals[0]?.agents}</dd></div><div><dt>最终 Agent 上限</dt><dd>{config.finalAgents}</dd></div>
