...
import type { ArchiveGodChoice, DetailItem, GodConfig, Relation, SectionConfig, SupplicantConfig } from "./GodArchiveTypes";
...
function allEntries(config: GodConfig) {
 return [...config.powers, ...(config.supplicant?.abilities ?? []), ...(config.drawCards?.items ?? []), ...config.sections.flatMap(section => section.items)];
}
