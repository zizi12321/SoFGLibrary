...
import type { ArchiveGodChoice, DetailItem, GodConfig, PowerItem, Relation, SectionConfig, SupplicantConfig } from "./GodArchiveTypes";
...
function allEntries(config: GodConfig) {
 return [...config.powers, ...(config.drawPowers ?? []), ...(config.supplicant?.abilities ?? []), ...(config.drawCards?.items ?? []), ...config.sections.flatMap(section => section.items)];
}
...
function AbilityList({ config, abilities }: { config: GodConfig; abilities: DetailItem[] }) {
 return <div className="ability-list">{abilities.map((ability, index) => <div className="ability" id={anchorFor(ability.name)} key={ability.name}><span>{String(index + 1).padStart(2, "0")}</span><div><h4>{ability.name}</h4><p><RichText config={config} text={ability.text} exclude={ability.name} /></p></div></div>)}</div>;
}
...
function PowerSection({ config, powers, id, index, title, openEntries, onToggle, hideSeal = false, hideCost = false }: { config: GodConfig; powers: PowerItem[]; id: string; index: string; title: string; openEntries: Set<string>; onToggle: (id: string) => void; hideSeal?: boolean; hideCost?: boolean }) {
 return <section id={id} className="section powers-section records-section"><div className="plain-heading"><p className="section-index">{index} / {title}</p><h2>{title}</h2><div className="power-legend"><span><Eye size={16} />效果</span><span><Shield size={16} />限制</span></div></div><div className="expandable-table powers-table">{powers.map(power => { const entryId = power.id ? "entry-" + power.id : anchorFor(power.name); return <ExpandableRow key={entryId} config={config} entryId={entryId} name={power.name} seal={hideSeal ? undefined : power.seal} cost={hideCost ? undefined : power.cost} image={power.icon} open={openEntries.has(entryId)} onToggle={onToggle}><div className="expanded-section"><h4>具体效果</h4><p><RichText config={config} text={power.effect} exclude={power.name} /></p></div><div className="expanded-section"><h4>释放限制</h4><p><RichText config={config} text={power.limit} exclude={power.name} /></p></div><RelationGroup config={config} title="来源" items={config.relations?.[power.name]?.sources} /><RelationGroup config={config} title="造成的效果" items={config.relations?.[power.name]?.effects} /></ExpandableRow>; })}</div></section>;
}
