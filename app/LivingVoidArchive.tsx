...
type DetailItem = { name: string; text: string; image?: string; images?: string[]; seal?: number; meta?: string; statLine?: string; location?: string; preferenceText?: string; id?: string };
...
const heroTasks: DetailItem[] = [
  { name: "Seal Rift", image: "void_rift_expand.png", location: "有 Hungry Rift 的地点。", meta: "Lore", statLine: "Complexity: 20–40　Profile: 裂隙历史最高强度　Menace: 该值 × 0.2　XP: 36–60", preferenceText: "喜欢或极端喜欢危险 会提高执行意愿，厌恶危险 则会降低。英雄 Awareness 与世界恐慌越高，意愿越强；Lore 不高于 2 时，只保留约四分之一的任务吸引力。人物 Shadow 会进一步压低意愿。", text: "英雄削减当地 Hungry Rift 60%；降至 0% 或以下时彻底移除。地点 Shadow 会把复杂度从 20 最多提高到 40。任务显示的 Profile 与 Menace取决于裂隙有史以来达到的最大强度，因此即使裂隙被压低，人类仍会记得它曾经的 Menace。" },
  { name: "Fund Reality Anchor", image: "void_riftcloser.png", id: "hero-fund-reality-anchor", location: "有 Reality Anchor 的地点。", meta: "Other", statLine: "Complexity: 1　Profile: 40　Menace: 0　XP: 3", preferenceText: "厌恶或极端厌恶 Gold 会提高执行意愿，喜欢 Gold 则会降低。世界恐慌与设施受损会提高意愿；若英雄持有的 Gold 不多于设施现有资金，意愿降低 100；设施属于黑暗势力时再降低 100。人物 Shadow 会按比例削弱仍为正数的意愿。", text: "英雄把身上的全部 Gold 交给 Reality Anchor，优先修复损伤，剩余部分成为维护资金；完成时增加 5 Profile。至少需要持有 1 Gold。" },
  { name: "Seal Rupture", image: "void_CO_CloseRupture.png", location: "有 World Rupture 的地点。", meta: "Lore", statLine: "Complexity: 50　Profile: 100　Menace: 0　Danger: 50　XP: 72", preferenceText: "没有直接关联的喜好或厌恶标签。该任务只允许冷却完毕的 Chosen One 执行；世界恐慌、附近未被 Shadow 覆盖的人类聚居地和 World Rupture 数量都会提高意愿。世界上达到 8 个裂口时获得极高优先级，但第 9 封印破除后执行意愿会被强制清零。", text: "Chosen One 进行的引导任务，完成后彻底关闭 World Rupture，并移除其 Draining Warmth 等裂口附属修正。引导期间可通过远程地术攻击、直接攻击或迫使其撤退来打断。" },
];
...
const religiousTasks: DetailItem[] = [
  { name: "Holy: Open Rift", image: "void_riftmodifier.png", location: "对应 Holy Order 的 Temple 所在地点。", meta: "Lore", statLine: "Complexity: 40　Profile: 40　Menace: 0　XP: 60", preferenceText: "喜欢或极端喜欢 Religion 会提高侍僧执行意愿，厌恶 Religion 则会降低。Seekers of the New World 为 −1 时额外增加 50 意愿，为 −2 时增加 100。", text: "教团侍僧在本教神庙所在地建立 50% Hungry Rift；若当地已有 Hidden Voidstone，则消耗它并把初始强度提高到 100%。完成时增加 10 Profile 与 10 Menace。只有对应 Holy Order 的侍僧可执行，且教义必须处于 Elder 影响状态。" },
];

const rulerActions: DetailItem[] = [
  { name: "Construct Reality Anchor", image: "void_riftcloser.png", location: "没有 Reality Anchor 的人类聚居地。", text: "完全 Awareness 的人类统治者花费 100 Gold、耗时 5 回合，在没有 Reality Anchor 的聚居地建造一座。世界恐慌、邻近或同地 Hungry Rift、高繁荣和 Alliance 身份会提高 AI 意愿；低繁荣与统治者 Shadow 会降低意愿。" },
  { name: "Fund Reality Anchor", image: "void_riftcloser.png", id: "ruler-fund-reality-anchor", location: "已有 Reality Anchor 的人类聚居地。", text: "完全 Awareness、至少持有 50 Gold 且不属于黑暗势力的当地统治者，耗时 3 回合，把当前 Gold 的 50% 交给 Reality Anchor。世界恐慌、附近裂隙、设施损伤和 Alliance 身份会提高执行意愿。" },
];
...
function DetailGrid({ items, openEntries, onToggle, media = true }: { items: DetailItem[]; openEntries: Set<string>; onToggle: (id: string) => void; media?: boolean }) {
  return <div className="expandable-table">{items.map((item) => { const entryId = item.id ? `entry-${item.id}` : anchorFor(item.name); const rel = relations[item.id ?? item.name]; return <ExpandableRow key={entryId} entryId={entryId} name={item.name} seal={item.seal} image={media ? item.image ?? item.images?.[0] : undefined} open={openEntries.has(entryId)} onToggle={onToggle}>{item.meta && <div className="expanded-meta">{item.meta}</div>}{item.location && <div className="task-location"><b>执行地点</b><RichText text={item.location} exclude={item.name} /></div>}{item.statLine && <div className="task-stat-line">{item.statLine}</div>}<p><RichText text={item.text} exclude={item.name} /></p>{item.preferenceText && <div className="preference-note"><b>执行倾向</b><p><RichText text={item.preferenceText} exclude={item.name} /></p></div>}<RelationGroup title="来源" items={rel?.sources} /><RelationGroup title="造成的效果" items={rel?.effects} /></ExpandableRow>; })}</div>;
}
export default function LivingVoidArchive({ onGodChange }: { onGodChange: (god: GodChoice) => void }) {
  ...
  return <main ref={root} className={`site-shell void-theme ${sidebarHidden ? "sidebar-hidden" : ""}`} onClickCapture={(event) => { const anchor = (event.target as HTMLElement).closest('a[href^="#entry-"]'); if (anchor) setOpenEntries((current) => new Set(current).add(anchor.getAttribute("href")!.slice(1))); }}>
    <aside className="sidebar"><div className="sidebar-head"><div className="sidebar-brand god-switcher"><span className="brand-mark"><GodMark god="living-void" /></span><label><select value="living-void" onChange={(event) => onGodChange(event.target.value as GodChoice)} aria-label="切换神祇"><option value="she-who-will-feast">SHE WHO WILL FEAST</option><option value="iastur">IASTUR</option><option value="vinerva">VINERVA</option><option value="ophanim">OPHANIM</option><option value="mammon">MAMMON</option><option value="broken-maker">THE BROKEN MAKER</option><option value="evil-beneath">THE EVIL BENEATH</option><option value="deaths-games">DEATH'S GAMES</option><option value="cordyceps">CORDYCEPS</option><option value="kishi">KISHI</option><option value="living-void">LIVING VOID</option><option value="chandalor">CHANDALOR</option></select><small>神祇资料库</small></label></div><button className="sidebar-toggle" type="button" onClick={() => setSidebarHidden((value) => !value)} aria-label={sidebarHidden ? "展开侧边栏" : "暂时隐藏侧边栏"} title={sidebarHidden ? "展开侧边栏" : "暂时隐藏侧边栏"}>{sidebarHidden ? <PanelLeftOpen size={17} /> : <PanelLeftClose size={17} />}</button></div><button className="sidebar-index-link" type="button" onClick={() => onGodChange("index")}>← 返回神祇索引</button><nav className="sidebar-nav" aria-label="页面目录">{nav.map(([index, id, label]) => <a href={`#${id}`} key={id}><span>{index}</span><b>{label}</b></a>)}</nav><div className="sidebar-bulk"><button type="button" onClick={() => setOpenEntries(new Set(allExpandableIds))}>全部展开</button><button type="button" onClick={() => setOpenEntries(new Set())}>全部收起</button></div><p className="sidebar-note">悬浮带下划线的名称可查看说明，点击可跳转至详情。</p></aside>
    <div className="content-shell">
      <header id="top" className="hero"><div className="hero-backdrop"><Image src="./living-void/god_background.png" alt="" fill priority sizes="100vw" /></div><div className="hero-copy"><p className="eyebrow"><span>神祇档案 05</span><span>Living Void God</span></p><div className="hero-title-wrap"><h1><span className="title-mask"><span className="hero-line">The Living Void</span></span></h1></div><div className="hero-facts"><div><b>400</b><span>回合苏醒</span></div><div><b>6</b><span>最终 Agent 上限</span></div><div><b>100%</b><span>苏醒时世界恐慌</span></div></div></div><div className="hero-art"><div className="portrait-frame hero-portrait reveal-image"><Image src="./living-void/god_portrait.png" alt="The Living Void 神祇立绘" fill priority sizes="(max-width: 900px) 100vw, 46vw" /></div></div></header>
      <section id="loop" className="section overview-section"><div className="section-heading"><p className="section-index">01 / 基础信息与核心玩法</p><h2>基础信息与核心玩法</h2></div><div className="overview-layout"><article className="overview-main"><h3>核心玩法</h3><ol className="core-sequence">
        ...
        <li><span>06</span><p>苏醒后施放 <CrossReference name="Vacuum Collapse" />，从 <CrossReference name="Great Wound" /> 与所有 <CrossReference name="World Rupture" /> 同时向外吞噬地图。</p></li>
      </ol><div className="overview-subsection"><h3><CrossReference name="Hungry Rift" /> 的成长与反制</h3><ul className="source-list"><li>当地每 1% Devastation 每回合约提供 0.01% 裂隙成长；城市废墟按 300% Devastation 计算。</li><li>Geomantic Locus 每回合被吞噬 10 点，作为裂隙成长的另一来源。</li><li>受 Elder 影响的神庙每回合提供 5% 或 10% 成长。</li><li>正常运作的远方 <CrossReference name="Reality Anchor" /> 每座削减 0.2%；与裂隙同地时削减 5%。</li><li>英雄执行 <CrossReference name="Seal Rift" />，每次削减 60%，地点 Shadow 会拖慢任务并降低扩张时的 Profile。</li></ul></div><div className="overview-subsection"><h3>苏醒后的扩散</h3><p><CrossReference name="Vacuum Collapse" /> 首次覆盖各入口周围半径 2 的范围，以后每次增加 2。多建 <CrossReference name="World Rupture" /> 不但增加同时扩散的起点，也会提高施放后的神力返还；但 World Rupture 仍可能被 Chosen One 关闭。</p></div></article><aside className="basic-facts"><h3>基础信息</h3><dl><div><dt>封印解锁方式</dt><dd>常规回合解锁</dd></div><div><dt>最大回合数</dt><dd>常规 500 回合</dd></div><div><dt>苏醒回合</dt><dd>第 400 回合</dd></div><div><dt>苏醒时世界恐慌</dt><dd>100%</dd></div><div><dt>初始 Agent 上限</dt><dd>2</dd></div><div><dt>最终 Agent 上限</dt><dd>6</dd></div></dl><div className="special-victory"><h3>特殊胜利</h3><p>无</p></div></aside></div></section>
      <section id="seals" className="section seals-section"><div className="section-heading row-heading"><div><p className="section-index">02 / 封印进度</p><h2>封印、解锁回合与 Agent 上限</h2></div></div><div className="seal-table"><div className="seal-head"><span>封印</span><span>回合</span><span>Agent</span><span>神力/恢复</span><span>本阶段内容</span></div>{seals.map((item) => <div className="seal-row" key={item.seal}><span className="seal-number">{item.seal}</span><span className="turn">{item.turn}</span><span className="agent-count">{item.agents}</span><span className="power-gain">{`${item.seal + 1} / ${(0.035 * (item.seal + 1)).toFixed(3)}`}</span><span className="seal-reward">{item.reward ? item.reward.split("、").map((reward, index) => <span className="seal-reference-item" key={reward}>{reward === "苏醒" ? reward : <CrossReference name={reward} />}{index < item.reward.split("、").length - 1 && <i>、</i>}</span>) : null}</span></div>)}</div><div className="seal-formula"><b>神力恢复公式</b><code>0.035 ×（已破封印数 + 1）× 难度缩放</code><span>表中按难度缩放为 1 计算。Living Void 没有覆写本体神力恢复规则。</span></div></section>      <section id="powers" className="section powers-section records-section"><div className="powers-intro plain-heading"><p className="section-index">03 / 神力</p><h2>神力</h2><div className="power-legend"><span><Eye size={16} />效果</span><span><Shield size={16} />限制</span></div></div><div className="expandable-table powers-table">{powers.map((power) => { const id=anchorFor(power.name); return <ExpandableRow key={id} entryId={id} name={power.name} seal={power.seal} cost={power.cost} image={power.icon} open={openEntries.has(id)} onToggle={toggleEntry}><div className="expanded-section"><h4>具体效果</h4><p><RichText text={power.effect} exclude={power.name} /></p></div><div className="expanded-section"><h4>释放限制</h4><p><RichText text={power.limit} exclude={power.name} /></p></div><RelationGroup title="造成的效果" items={powerEffects[power.name]} /></ExpandableRow>; })}</div></section>
