...
const challenges: DetailItem[] = [
  { name: "Something from Nothing", image: "void_stone_heal.png", meta: "Other", statLine: "复杂度 1　暴露度 0　危险 0　经验 3（标准难度）", text: "消耗一枚 Voidstone，使 Agent 恢复 2 HP，并让其所有现存随从各恢复 2 HP，均不超过最大值。" },
  { name: "Plant Voidstone", image: "void_stone.png", meta: "Intrigue", statLine: "复杂度 25　暴露度 0　危险 0　经验 42（标准难度）", text: "消耗一枚 Voidstone，在当地建立 Hidden Voidstone；完成时获得 5 Profile 与 4 Menace。要求位于渗透度至少 50% 的人类聚居地，且当地没有 Hidden Voidstone 或 Hungry Rift。" },

  { name: "Nothing from Something", image: "void_stone_get.png", meta: "Other", statLine: "复杂度 1　暴露度 0　危险 0　经验 3（标准难度）", text: "在 Great Wound 或 World Rupture 牺牲 2 HP，获得一枚 Voidstone。只有当前 HP 大于 2 时才可执行。" },
  { seal: 4, name: "Expand Rift", image: "void_rift_expand.png", meta: "Lore", statLine: "复杂度 25　暴露度 0　危险 0　经验 42（标准难度）", text: "使当地 Hungry Rift 增加 40%。完成时基础增加 5 Profile 与 8 Menace；地点 Shadow 会使两者最多降低 35%，最终取整。" },
  { name: "Pillage Anchor", image: "void_towerraid.png", meta: "Might", statLine: "复杂度 20 + 地点安全 × 5　暴露度 0　危险 0　经验 36 起（标准难度）", text: "夺走 Reality Anchor 的全部维护资金，并造成 Agent 当前 Might × 100 Gold 的设施损伤，使其停止压制裂隙；完成时增加 6 Profile 与 12 Menace。" },
];

const heroTasks: DetailItem[] = [
  { name: "Seal Rift", image: "void_rift_expand.png", meta: "Lore", statLine: "复杂度 20–40　暴露度为裂隙历史最高强度　危险为该值的 20%　经验 36–60（标准难度）", preferenceText: "喜欢或极端喜欢危险 会提高执行意愿，厌恶危险 则会降低。英雄 Awareness 与世界恐慌越高，意愿越强；Lore 不高于 2 时，只保留约四分之一的任务吸引力。人物 Shadow 会进一步压低意愿。", text: "英雄削减当地 Hungry Rift 60%；降至 0% 或以下时彻底移除。地点 Shadow 会把复杂度从 20 最多提高到 40。任务显示的暴露度与危险取决于裂隙有史以来达到的最大强度，因此即使裂隙被压低，人类仍会记得它曾经的威胁。" },
  { name: "Fund Reality Anchor", image: "void_riftcloser.png", id: "hero-fund-reality-anchor", meta: "Other", statLine: "复杂度 1　暴露度 40　危险 0　经验 3（标准难度）", preferenceText: "厌恶或极端厌恶 Gold 会提高执行意愿，喜欢 Gold 则会降低。世界恐慌与设施受损会提高意愿；若英雄持有的 Gold 不多于设施现有资金，意愿降低 100；设施属于黑暗势力时再降低 100。人物 Shadow 会按比例削弱仍为正数的意愿。", text: "英雄把身上的全部 Gold 交给 Reality Anchor，优先修复损伤，剩余部分成为维护资金；完成时增加 5 Profile。至少需要持有 1 Gold。" },
  { name: "Seal Rupture", image: "void_CO_CloseRupture.png", meta: "Lore", statLine: "复杂度 50　暴露度 100　危险 0　经验 72（标准难度）", preferenceText: "没有直接关联的喜好或厌恶标签。该任务只允许冷却完毕的 Chosen One 执行；世界恐慌、附近未被 Shadow 覆盖的人类聚居地和 World Rupture 数量都会提高意愿。世界上达到 8 个裂口时获得极高优先级，但第 9 封印破除后执行意愿会被强制清零。", text: "Chosen One 进行的引导任务，完成后彻底关闭 World Rupture，并移除其 Draining Warmth 等裂口附属修正。引导期间可通过远程地术攻击、直接攻击或迫使其撤退来打断。" },
];
...
export default function LivingVoidArchive({ onGodChange }: { onGodChange: (god: GodChoice) => void }) {
  ...
  return <main ref={root} className={`site-shell void-theme ${sidebarHidden ? "sidebar-hidden" : ""}`} onClickCapture={(event) => { const anchor = (event.target as HTMLElement).closest('a[href^="#entry-"]'); if (anchor) setOpenEntries((current) => new Set(current).add(anchor.getAttribute("href")!.slice(1))); }}>
    ...
    <div className="content-shell">
      ...
      </ol><div className="overview-subsection"><h3><CrossReference name="Hungry Rift" /> 的成长与反制</h3><ul className="source-list"><li>当地每 1% Devastation 每回合约提供 0.01% 裂隙成长；城市废墟按 300% Devastation 计算。</li><li>Geomantic Locus 每回合被吞噬 10 点，作为裂隙成长的另一来源。</li><li>受 Elder 影响的神庙每回合提供 5% 或 10% 成长。</li><li>正常运作的远方 <CrossReference name="Reality Anchor" /> 每座削减 0.2%；与裂隙同地时削减 5%。</li><li>英雄执行 <CrossReference name="Seal Rift" />，每次削减 60%，地点 Shadow 会拖慢任务并降低扩张时的暴露。</li></ul></div><div className="overview-subsection"><h3>苏醒后的扩散</h3><p><CrossReference name="Vacuum Collapse" /> 首次覆盖各入口周围半径 2 的范围，以后每次增加 2。多建 <CrossReference name="World Rupture" /> 不但增加同时扩散的起点，也会提高施放后的神力返还；但 World Rupture 仍可能被 Chosen One 关闭。</p></div></article><aside className="basic-facts"><h3>基础信息</h3><dl><div><dt>封印解锁方式</dt><dd>常规回合解锁</dd></div><div><dt>最大回合数</dt><dd>常规 500 回合</dd></div><div><dt>苏醒回合</dt><dd>第 400 回合</dd></div><div><dt>苏醒时世界恐慌</dt><dd>100%</dd></div><div><dt>初始 Agent 上限</dt><dd>2</dd></div><div><dt>最终 Agent 上限</dt><dd>6</dd></div></dl><div className="special-victory"><h3>特殊胜利</h3><p>无</p></div></aside></div></section>
