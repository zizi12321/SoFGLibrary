...
export default function LivingVoidArchive({ onGodChange }: { onGodChange: (god: GodChoice) => void }) {
  ...
  return <main ref={root} className={`site-shell void-theme ${sidebarHidden ? "sidebar-hidden" : ""}`} onClickCapture={(event) => { const anchor = (event.target as HTMLElement).closest('a[href^="#entry-"]'); if (anchor) setOpenEntries((current) => new Set(current).add(anchor.getAttribute("href")!.slice(1))); }}>
    ...
    <div className="content-shell">
      ...
      </ol><div className="overview-subsection"><h3><CrossReference name="Hungry Rift" /> 的成长与反制</h3><ul className="source-list"><li>当地每 1% Devastation 每回合约提供 0.01% 裂隙成长；城市废墟按 300% Devastation 计算。</li><li>Geomantic Locus 每回合被吞噬 10 点，作为裂隙成长的另一来源。</li><li>受 Elder 影响的神庙每回合提供 5% 或 10% 成长。</li><li>正常运作的远方 <CrossReference name="Reality Anchor" /> 每座削减 0.2%；与裂隙同地时削减 5%。</li><li>英雄执行 <CrossReference name="Seal Rift" />，每次削减 60%，地点 Shadow 会拖慢任务并降低扩张时的暴露。</li></ul></div><div className="overview-subsection"><h3>苏醒后的扩散</h3><p><CrossReference name="Vacuum Collapse" /> 首次覆盖各入口周围半径 2 的范围，以后每次增加 2。多建 <CrossReference name="World Rupture" /> 不但增加同时扩散的起点，也会提高施放后的神力返还；但 World Rupture 仍可能被 Chosen One 关闭。</p></div></article><aside className="basic-facts"><h3>基础信息</h3><dl><div><dt>封印解锁方式</dt><dd>常规回合解锁</dd></div><div><dt>最大回合数</dt><dd>常规 500 回合</dd></div><div><dt>苏醒回合</dt><dd>第 400 回合</dd></div><div><dt>苏醒时世界恐慌</dt><dd>100%</dd></div><div><dt>初始 Agent 上限</dt><dd>2</dd></div><div><dt>最终 Agent 上限</dt><dd>6</dd></div></dl><div className="omen-info"><h3>玩法重点</h3><p>前期重点是运输和埋藏 <CrossReference name="Voidstone" />；中期集中把少量 <CrossReference name="Hungry Rift" /> 推到 300%，避免被英雄逐个封闭；后期用多个 <CrossReference name="World Rupture" /> 提高 <CrossReference name="Vacuum Collapse" /> 的覆盖面和返还效率。</p></div></aside></div></section>
