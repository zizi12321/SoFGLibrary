...
type DetailItem = { name: string; text: string; image?: string; seal?: number; meta?: string; statLine?: string; location?: string; preferenceText?: string; id?: string; baseGame?: boolean };
...
const mechanics: DetailItem[] = [
  ...
  { name: "Challenge Danger", id: "mechanic-danger", baseGame: true, text: "挑战的危险值决定执行者在任务过程中遭遇负面结果的风险。Dangers in the Dark 会给一个地点内每项英雄或中立挑战分别增加 7 点附加危险，并且不会自然衰减。" },
];
...
const heroTasks: DetailItem[] = [
  { name: "Purge Fleeting Servant", image: "power-fleeting-servant.png", location: "有 Fleeting Servant 修正的地点。", meta: "Intrigue", statLine: "Complexity: 5　Profile: 30　Menace: 50　XP: 12", preferenceText: "任务带有 Shadow 负面标签：厌恶或极端厌恶 Shadow 的英雄更愿意处理它；喜欢 Shadow 的英雄意愿降低。除此之外没有专属固定意愿加值。", text: "英雄清除所在地的一项 Fleeting Servant 修正。完成任务本身只移除诱饵，不会产生额外地点收益。" },
];
...
function DetailGrid({ items, openEntries, onToggle, media = true }: { items: DetailItem[]; openEntries: Set<string>; onToggle: (id: string) => void; media?: boolean }) {
  return <div className="expandable-table">{items.map((item) => { const entryId = item.id ? `entry-${item.id}` : anchorFor(item.name); const rel = relations[item.name]; return <ExpandableRow key={entryId} entryId={entryId} name={item.name} seal={item.seal} image={media ? item.image : undefined} baseGame={item.baseGame} open={openEntries.has(entryId)} onToggle={onToggle}>{item.meta && <div className="expanded-meta">{item.meta}</div>}{item.location && <div className="task-location"><b>执行地点</b><RichText text={item.location} exclude={item.name} /></div>}{item.statLine && <div className="task-stat-line">{item.statLine}</div>}<p><RichText text={item.text} exclude={item.name} /></p>{item.preferenceText && <div className="preference-note"><b>执行倾向</b><p><RichText text={item.preferenceText} exclude={item.name} /></p></div>}<RelationGroup title="来源" items={rel?.sources} /><RelationGroup title="造成的效果" items={rel?.effects} /></ExpandableRow>; })}</div>;
}
...
export default function SheWhoWillFeastArchive({ onGodChange }: { onGodChange: (god: GodChoice) => void }) {
  ...
  return <main ref={root} className={`site-shell feast-theme ${sidebarHidden ? "sidebar-hidden" : ""}`} onClickCapture={(event) => { const anchor = (event.target as HTMLElement).closest('a[href^="#entry-"]'); if (anchor) setOpenEntries((current) => new Set(current).add(anchor.getAttribute("href")!.slice(1))); }}>
    ...
    <div className="content-shell">
      ...
      <section id="seals" className="section seals-section"><div className="section-heading row-heading"><div><p className="section-index">02 / 封印进度</p><h2>封印、解锁回合与 Agent 上限</h2></div></div><div className="seal-table"><div className="seal-head"><span>封印</span><span>回合</span><span>Agent</span><span>神力/恢复</span><span>本阶段内容</span></div>{seals.map((item) => <div className="seal-row" key={item.seal}><span className="seal-number">{item.seal}</span><span className="turn">{item.turn}</span><span className="agent-count">{item.agents}</span><span className="power-gain">{`${item.seal + 1} / ${(0.035 * (item.seal + 1)).toFixed(3)}`}</span><span className="seal-reward">{item.reward ? item.reward.split("、").map((reward, index) => <span className="seal-reference-item" key={reward}>{reward === "苏醒" ? reward : <CrossReference name={reward} />}{index < item.reward.split("、").length - 1 && <i>、</i>}</span>) : null}</span></div>)}</div><div className="seal-formula"><b>神力恢复公式</b><code>0.035 ×（已破封印数 + 1）× 难度缩放</code><span>表中按难度缩放为 1 计算。破封后的神力上限从 1 逐步提高到 10。</span></div></section>
