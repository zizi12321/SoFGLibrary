import{_ as e,d as t,f as n,g as r,h as i,l as a,r as o,t as s,u as c,v as l,y as u}from"./index-DUnwG3E7.js";var d=u(),f=[{id:`place-model`,name:`地点、兴趣点与行动入口`,text:`地点是地图上的聚居地或野外地点载体；兴趣点是附加在地点内的具体设施。
...
由具体的 Villikos 占领地点继承，不单独作为地图地点生成。`,meta:`未找到正常生成路径`}]}]}],m=p.flatMap(e=>e.sections),h=m.flatMap(e=>e.items),g=new Map(h.map(e=>[e.id,e])),_=new Map(m.flatMap(e=>e.items.map(t=>[t.id,e.title]))),v=e=>e.id?.startsWith(`location-`),y=e=>e.id?.startsWith(`location-sub-`)?`points-of-interest`:`locations`,b=e=>[...e.matchAll(/href="#entry-([^"]+)"/g)].map(e=>e[1]),x=e=>[...new Set(e.flatMap(e=>b(e.text)).filter(e=>e.startsWith(`place-task-`)))],S=e=>{let t=e.text.split(`

`).find(e=>e.startsWith(`对应地点 / 兴趣点`));return t?b(t).filter(e=>g.has(e)&&v(g.get(e))):[]},C=/^(?:共同行动|英雄任务|挑战|宗教任务|自主单位任务)：/,w=e=>e.split(`

`).filter(e=>!C.test(e)).join(`

`),T=e=>{let t=new Map;for(let n of e){let e=g.get(n);if(!e)continue;let r=_.get(n);t.set(r,[...t.get(r)??[],e])}return[...t].map(([e,t])=>({title:e,items:t}))};function E(e){return p.map(t=>{let n=t.sections.flatMap(e=>e.items).filter(t=>v(t)&&y(t)===e),r=t.sections.filter(e=>e.id.endsWith(`-extensions`)).flatMap(e=>e.items),i=new Set,a=n.map(n=>{let a=r.filter(e=>S(e).includes(n.id));a.forEach(e=>i.add(e.id));let o={...n,text:w(n.text)};return{id:`place-article-`+n.id,name:n.name,image:n.image,unplaced:t.sections.some(e=>e.id.endsWith(`-unplaced`)&&e.items.includes(n)),blocks:[{title:e===`locations`?`地点`:`兴趣点`,items:[o]},...a.length?[{title:`条件性行动入口`,items:a.map(e=>({...e,text:w(e.text)}))}]:[],...T(x([n,...a]))]}}),o=[...new Set(r.filter(e=>!i.has(e.id)).flatMap(S))].filter(t=>y(g.get(t))===e&&!n.some(e=>e.id===t));for(let e of o){let n=g.get(e),o=r.filter(t=>S(t).includes(e));o.forEach(e=>i.add(e.id)),a.push({id:t.id+`-extension-`+e,name:n.name+` · 扩展`,image:n.image,blocks:[{title:`条件性行动入口`,items:o.map(e=>({...e,text:w(e.text)}))},...T(x(o))]})}if(e===`locations`)for(let e of r.filter(e=>!S(e).length))a.push({id:`place-article-`+e.id,name:e.name,blocks:[{title:`条件性行动入口`,items:[{...e,text:w(e.text)}]},...T(x([e]))]});return{id:t.id,title:t.title,description:t.description,articles:a}}).filter(e=>e.articles.length)}var D=new Map,O=new Map([[`locations`,E(`locations`)],[`points-of-interest`,E(`points-of-interest`)]]),k=new Set([...O.values()].flatMap(e=>e.flatMap(e=>e.articles.flatMap(e=>e.blocks.flatMap(e=>e.items.map(e=>e.id))))));for(let e of p){let t=e.sections.flatMap(e=>e.items).filter(e=>e.id?.startsWith(`place-task-`)&&!k.has(e.id));if(!t.length)continue;let n=O.get(`locations`),r=n.find(t=>t.id===e.id);r||(r={id:e.id,title:e.title,description:e.description,articles:[]},n.push(r)),r.articles.push({id:e.id+`-additional-actions`,name:`其他地点行动`,blocks:T(t.map(e=>e.id))})}for(let e of O.values())e.sort((e,t)=>p.findIndex(t=>t.id===e.id)-p.findIndex(e=>e.id===t.id));var A=new Map;for(let[e,t]of O){let n=new Set;for(let r of t)for(let t of r.articles)for(let r of t.blocks)r.items=r.items.map(r=>{let i=r.id,a=n.has(i)?t.id+`--`+i:i;n.add(i);let o=A.get(i)??new Map;return o.has(e)||o.set(e,a),A.set(i,o),{...r,id:a}})}var j=e=>e.replace(/<CrossReference name="([^"]+)"[^>]*\/>/g,`$1`).replaceAll(`"`,`“`);for(let[e,t]of O){let n=(t,n=new Map)=>t.replace(/<CrossReference name="([^"]+)" href="#entry-([^"]+)"\s*\/>/g,(t,r,i)=>{let a=n.get(i)??A.get(i)?.get(e);if(a)return`<CrossReference name="`+r+`" href="#entry-`+a+`" />`;let o=[...A.get(i)?.entries()??[]][0],s=g.get(i);if(!o||!s)return t;let c=j([s.text,s.location&&`执行地点：`+s.location,s.meta,s.statLine,s.limit&&`执行条件：`+s.limit].filter(Boolean).join(`
`)),l=v(s)?y(s)===`locations`?`地点`:`兴趣点`:_.get(i)??``;return`<CrossReference name="`+r+`" href="?page=`+o[0]+`#entry-`+o[1]+`" meta="`+l+`" text="`+c+`"`+(s.image?` image="`+s.image+`"`:``)+` />`}),r=(e,t)=>({...e,text:n(e.text,t),location:e.location?n(e.location,t):void 0,limit:e.limit?n(e.limit,t):void 0});for(let e of t)for(let t of e.articles){let e=new Map(t.blocks.flatMap(e=>e.items.map(e=>[e.id.split(`--`).at(-1),e.id])));for(let n of t.blocks)n.items=n.items.map(t=>r(t,e))}let i=f.map(e=>r({...e,text:e.id===`place-model`?`地点是地图上的聚居地或野外地点载体；兴趣点是附加在地点内的具体设施。
每个条目分别列出自身提供的挑战与任务；地点修正、单位、神祇与 Mod 追加的行动另注明条件。实际能否执行还取决于执行者、渗透、资源、信仰及世界状态。`:e.text})),a=[{id:`place-guide`,title:`阅读说明`,items:i},...t.flatMap(e=>e.articles.flatMap(e=>e.blocks.map((t,n)=>({id:e.id+`-block-`+n,title:t.title,items:t.items}))))],o={};for(let[t,n]of A)if(!n.has(e)){let[e,r]=[...n][0];o[`entry-`+t]=`?page=`+e+`#entry-`+r}D.set(e,{kind:e,title:e===`locations`?`地点`:`兴趣点`,sources:t,intro:i,redirects:o,config:{id:e,assetDir:`locations`,autoLink:!1,powers:[],sections:a}})}function M(e){return D.get(e)}var N=l();function P({onReturn:l,kind:u=`locations`}){let f=M(u),{config:p,title:m,sources:h,intro:g}=f,_=p.sections.flatMap(e=>e.items.map(e=>`entry-`+e.id)),{sidebarHidden:v,setSidebarHidden:y,closeMobileSidebar:b,onSidebarClick:x}=e(),[S,C]=(0,d.useState)(new Set),w=e=>C(t=>new Set(t).add(e)),T=e=>C(t=>{let n=new Set(t);return n.has(e)?n.delete(e):n.add(e),n}),E=i(w);(0,d.useEffect)(()=>{let e=()=>{let e=f.redirects[window.location.hash.slice(1)];e&&window.location.replace(e)};return e(),window.addEventListener(`hashchange`,e),()=>window.removeEventListener(`hashchange`,e)},[f]);let D=[{name:`阅读说明`,entries:[{href:`#place-guide`,label:m}]},...h.map(e=>({name:e.title,entries:e.articles.map(e=>({href:`#`+e.id,label:e.name}))}))],O=u===`locations`?{page:`points-of-interest`,title:`兴趣点`}:{page:`locations`,title:`地点`};return(0,N.jsxs)(`main`,{className:`site-shell base-modifiers-theme locations-theme`+(v?` sidebar-hidden`:``),onClick:E,children:[(0,N.jsx)(r,{hidden:v,onClose:b}),(0,N.jsxs)(`aside`,{className:`sidebar`,onClick:x,children:[(0,N.jsxs)(`div`,{className:`sidebar-head`,children:[(0,N.jsxs)(`div`,{className:`sidebar-brand`,children:[(0,N.jsx)(`span`,{className:`brand-mark`,children:u===`locations`?(0,N.jsx)(t,{size:17}):(0,N.jsx)(n,{size:17})}),(0,N.jsxs)(`span`,{children:[(0,N.jsx)(`b`,{children:m}),(0,N.jsx)(`small`,{children:`其他信息`})]})]}),(0,N.jsx)(`button`,{className:`sidebar-toggle`,type:`button`,"aria-expanded":!v,"aria-label":v?`展开侧边栏`:`收起侧边栏`,onClick:()=>y(e=>!e),children:v?(0,N.jsx)(a,{size:17}):(0,N.jsx)(c,{size:17})})]}),(0,N.jsx)(`button`,{className:`sidebar-index-link`,type:`button`,onClick:l,children:`← 返回索引页`}),(0,N.jsxs)(`a`,{className:`sidebar-index-link`,href:`?page=`+O.page,children:[`查看`,O.title,` →`]}),(0,N.jsxs)(`div`,{className:`sidebar-bulk`,children:[(0,N.jsx)(`button`,{type:`button`,onClick:()=>C(new Set(_)),children:`全部展开`}),(0,N.jsx)(`button`,{type:`button`,onClick:()=>C(new Set),children:`全部收起`})]}),(0,N.jsx)(s,{groups:D,label:m+`导航`})]}),(0,N.jsxs)(`div`,{className:`content-shell`,children:[(0,N.jsxs)(`header`,{id:`top`,className:`modifier-library-header`,children:[(0,N.jsx)(`h1`,{children:m}),(0,N.jsxs)(`p`,{children:[u===`locations`?`每个地点单独分栏，列出其介绍、可能配置的兴趣点、挑战与任务。`:`每个兴趣点单独分栏，列出其出现地点、功能、挑战与任务。`,`先列游戏本体，再按 Mod 分栏；包括神祇限定内容。`]})]}),(0,N.jsxs)(`section`,{className:`section location-guide`,id:`place-guide`,children:[(0,N.jsx)(`h2`,{children:`阅读说明`}),(0,N.jsx)(o,{config:p,items:g,openEntries:S,onToggle:T})]}),h.map(e=>(0,N.jsxs)(`div`,{className:`location-collection`,id:e.id,children:[(0,N.jsxs)(`header`,{className:`location-collection-heading`,children:[(0,N.jsx)(`p`,{className:`section-index`,children:[`places-base`,`places-cordyceps`].includes(e.id)?`游戏本体`:`Mod`}),(0,N.jsx)(`h2`,{children:e.title})]}),e.articles.map(e=>(0,N.jsxs)(`section`,{className:`section location-article`,id:e.id,children:[(0,N.jsxs)(`header`,{className:`location-article-heading`,children:[e.image&&(0,N.jsx)(`img`,{src:e.image,alt:``,loading:`lazy`}),(0,N.jsxs)(`div`,{children:[(0,N.jsx)(`h3`,{children:e.name}),e.unplaced&&(0,N.jsx)(`p`,{children:`未找到正常生成路径`})]})]}),e.blocks.map((e,t)=>(0,N.jsxs)(`div`,{className:`location-article-block`,children:[(0,N.jsx)(`h4`,{children:[`地点`,`兴趣点`].includes(e.title)?`介绍`:e.title}),(0,N.jsx)(o,{config:p,items:e.items,openEntries:S,onToggle:T})]},t))]},e.id))]},e.id)),(0,N.jsx)(`footer`,{className:`archive-footer`,children:(0,N.jsx)(`button`,{className:`sidebar-index-link`,type:`button`,onClick:l,children:`← 返回索引页`})})]})]})}export{P as default};