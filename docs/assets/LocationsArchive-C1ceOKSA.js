import{_ as e,d as t,f as n,g as r,h as i,l as a,r as o,t as s,u as c,v as l,y as u}from"./index-t2dZunk2.js";var d=u(),f=[{id:`place-model`,name:`地点、兴趣点与行动入口`,text:`地点是地图上的聚居地或野外地点载体；兴趣点是附加在地点内的具体设施。
...
<CrossReference name="`+s.name+`" href="#entry-`+n+`" />`+r})}return{...e,sections:t}}),b=y.map(e=>({...e,description:e.id===`places-base`?`人类、精灵、矮人、Orc、Deep Ones 与奇观相关的通用地点和兴趣点。`:e.description,sections:e.sections.map(e=>({...e,items:e.items.filter(e=>!h[e.id]&&e.id!==`place-hook-kalastrophe-defile`)})).filter(e=>e.items.length)})).filter(e=>e.sections.length),x=y.flatMap(e=>e.sections),S=x.flatMap(e=>e.items),C=new Map(S.map(e=>[e.id,e])),w=new Map(x.flatMap(e=>e.items.map(t=>[t.id,e.title]))),T=e=>e.id?.startsWith(`location-`),E=e=>e.id?.startsWith(`location-sub-`)?`points-of-interest`:`locations`,D=e=>[...e.matchAll(/href="#entry-([^"]+)"/g)].map(e=>e[1]),O=e=>[...new Set(e.flatMap(e=>D(e.text)).filter(e=>/^place-(?:task|ruler)-/.test(e)&&!h[e]))],k=e=>{let t=e.text.split(`

`).find(e=>e.startsWith(`对应地点 / 兴趣点`));return t?D(t).filter(e=>C.has(e)&&T(C.get(e))):[]},A=/^(?:共同行动|英雄任务|挑战|宗教任务|自主单位任务|统治者行动)：/,j=e=>e.split(`

`).filter(e=>!A.test(e)).join(`

`),M=e=>{let t=new Map;for(let n of e){let e=C.get(n);if(!e)continue;let r=w.get(n);t.set(r,[...t.get(r)??[],e])}return[...t].map(([e,t])=>({title:e,items:t}))};function N(e){return b.map(t=>{let n=t.sections.flatMap(e=>e.items).filter(t=>T(t)&&E(t)===e),r=t.sections.filter(e=>e.id.endsWith(`-extensions`)).flatMap(e=>e.items),i=new Set,a=n.map(n=>{let a=r.filter(e=>k(e).includes(n.id));a.forEach(e=>i.add(e.id));let o={...n,text:j(n.text)};return{id:`place-article-`+n.id,name:n.name,image:n.image,unplaced:t.sections.some(e=>e.id.endsWith(`-unplaced`)&&e.items.includes(n)),blocks:[{title:e===`locations`?`地点`:`兴趣点`,items:[o]},...a.length?[{title:`条件性行动入口`,items:a.map(e=>({...e,text:j(e.text)}))}]:[],...M(O([n,...a]))]}}),o=[...new Set(r.filter(e=>!i.has(e.id)).flatMap(k))].filter(t=>E(C.get(t))===e&&!n.some(e=>e.id===t));for(let e of o){let n=C.get(e),o=r.filter(t=>k(t).includes(e));o.forEach(e=>i.add(e.id)),a.push({id:t.id+`-extension-`+e,name:n.name+` · 扩展`,image:n.image,blocks:[{title:`条件性行动入口`,items:o.map(e=>({...e,text:j(e.text)}))},...M(O(o))]})}if(e===`locations`)for(let e of r.filter(e=>!k(e).length))a.push({id:`place-article-`+e.id,name:e.name,blocks:[{title:`条件性行动入口`,items:[{...e,text:j(e.text)}]},...M(O([e]))]});return{id:t.id,title:t.title,description:t.description,articles:a}}).filter(e=>e.articles.length)}var P=new Map,F=new Map([[`locations`,N(`locations`)],[`points-of-interest`,N(`points-of-interest`)]]),I=new Set([...F.values()].flatMap(e=>e.flatMap(e=>e.articles.flatMap(e=>e.blocks.flatMap(e=>e.items.map(e=>e.id))))));for(let e of b){let t=e.sections.flatMap(e=>e.items).filter(e=>e.id?.startsWith(`place-task-`)&&!I.has(e.id));if(!t.length)continue;let n=F.get(`locations`),r=n.find(t=>t.id===e.id);r||(r={id:e.id,title:e.title,description:e.description,articles:[]},n.push(r)),r.articles.push({id:e.id+`-additional-actions`,name:`其他地点行动`,blocks:M(t.map(e=>e.id))})}for(let e of F.values())e.sort((e,t)=>b.findIndex(t=>t.id===e.id)-b.findIndex(e=>e.id===t.id));for(let[e,t]of F){let n=new Set(m[e]);for(let e of t){let t=new Map;for(let r of e.articles){for(let e of r.blocks)e.items=e.items.filter(r=>{if(!n.has(r.id))return!0;let i=t.get(e.title)??{title:e.title,items:[]};return i.items.some(e=>e.id===r.id)||i.items.push(r),t.set(e.title,i),!1});r.blocks=r.blocks.filter(e=>e.items.length)}e.articles=e.articles.filter(e=>e.blocks.length),t.size&&e.articles.unshift({id:e.id+`-common-actions`,name:`共通`,blocks:[...t.values()]})}}var L=new Map;for(let[e,t]of F){let n=new Set;for(let r of t)for(let t of r.articles)for(let r of t.blocks)r.items=r.items.map(r=>{let i=r.id,a=n.has(i)?t.id+`--`+i:i;n.add(i);let o=L.get(i)??new Map;return o.has(e)||o.set(e,a),L.set(i,o),{...r,id:a}})}var R=e=>e.replace(/<CrossReference name="([^"]+)"[^>]*\/>/g,`$1`).replaceAll(`"`,`“`);for(let[e,t]of F){let n=(t,n=new Map)=>t.replace(/<CrossReference name="([^"]+)" href="#entry-([^"]+)"\s*\/>/g,(t,r,i)=>{if(v.has(i))return r;let a=n.get(i)??L.get(i)?.get(e);if(a)return`<CrossReference name="`+r+`" href="#entry-`+a+`" />`;let o=[...L.get(i)?.entries()??[]][0],s=h[i],c=C.get(i);if(!o&&!s||!c)return t;let l=R([c.text,c.location&&`执行地点：`+c.location,c.meta,c.statLine,c.limit&&`执行条件：`+c.limit].filter(Boolean).join(`
`)),u=T(c)?E(c)===`locations`?`地点`:`兴趣点`:w.get(i)??``;return`<CrossReference name="`+r+`" href="`+(s?.href??`?page=`+o[0]+`#entry-`+o[1])+`" meta="`+u+`" text="`+l+`"`+(c.image?` image="`+c.image+`"`:``)+` />`}),r=(e,t)=>({...e,text:n(e.text,t),location:e.location?n(e.location,t):void 0,limit:e.limit?n(e.limit,t):void 0});for(let e of t)for(let t of e.articles){let e=new Map(t.blocks.flatMap(e=>e.items.map(e=>[e.id.split(`--`).at(-1),e.id])));for(let n of t.blocks)n.items=n.items.map(t=>r(t,e))}let i=f.map(e=>r({...e,text:e.id===`place-model`?`地点是地图上的聚居地或野外地点载体；兴趣点是附加在地点内的具体设施。
每个条目分别列出自身提供的挑战与任务；地点修正、单位、神祇与 Mod 追加的行动另注明条件。实际能否执行还取决于执行者、渗透、资源、信仰及世界状态。`:e.text})),a=[{id:`place-guide`,title:`阅读说明`,items:i},...t.flatMap(e=>e.articles.flatMap(e=>e.blocks.map((t,n)=>({id:e.id+`-block-`+n,title:t.title,items:t.items}))))],o=Object.fromEntries(Object.entries(h).map(([e,t])=>[`entry-`+e,t.href]));for(let[t,n]of L)if(!n.has(e)){let[e,r]=[...n][0];o[`entry-`+t]=`?page=`+e+`#entry-`+r}P.set(e,{kind:e,title:e===`locations`?`地点`:`兴趣点`,sources:t,intro:i,redirects:o,config:{id:e,assetDir:`locations`,autoLink:!1,powers:[],sections:a}})}function z(e){return P.get(e)}var B=l();function V({onReturn:l,kind:u=`locations`}){let f=z(u),{config:p,title:m,sources:h,intro:g}=f,_=p.sections.flatMap(e=>e.items.map(e=>`entry-`+e.id)),{sidebarHidden:v,setSidebarHidden:y,closeMobileSidebar:b,onSidebarClick:x}=e(),[S,C]=(0,d.useState)(new Set),w=e=>C(t=>new Set(t).add(e)),T=e=>C(t=>{let n=new Set(t);return n.has(e)?n.delete(e):n.add(e),n}),E=i(w);(0,d.useEffect)(()=>{let e=()=>{let e=f.redirects[window.location.hash.slice(1)];e&&window.location.replace(e)};return e(),window.addEventListener(`hashchange`,e),()=>window.removeEventListener(`hashchange`,e)},[f]);let D=[{name:`阅读说明`,entries:[{href:`#place-guide`,label:m}]},...h.map(e=>({name:e.title,entries:e.articles.map(e=>({href:`#`+e.id,label:e.name}))}))];return(0,B.jsxs)(`main`,{className:`site-shell base-modifiers-theme locations-theme`+(v?` sidebar-hidden`:``),onClick:E,children:[(0,B.jsx)(r,{hidden:v,onClose:b}),(0,B.jsxs)(`aside`,{className:`sidebar`,onClick:x,children:[(0,B.jsxs)(`div`,{className:`sidebar-head`,children:[(0,B.jsxs)(`div`,{className:`sidebar-brand`,children:[(0,B.jsx)(`span`,{className:`brand-mark`,children:u===`locations`?(0,B.jsx)(t,{size:17}):(0,B.jsx)(n,{size:17})}),(0,B.jsxs)(`span`,{children:[(0,B.jsx)(`b`,{children:m}),(0,B.jsx)(`small`,{children:`其他信息`})]})]}),(0,B.jsx)(`button`,{className:`sidebar-toggle`,type:`button`,"aria-expanded":!v,"aria-label":v?`展开侧边栏`:`收起侧边栏`,onClick:()=>y(e=>!e),children:v?(0,B.jsx)(a,{size:17}):(0,B.jsx)(c,{size:17})})]}),(0,B.jsx)(`a`,{className:`sidebar-index-link`,href:`./`,children:`← 返回索引页`}),(0,B.jsxs)(`div`,{className:`sidebar-bulk`,children:[(0,B.jsx)(`button`,{type:`button`,onClick:()=>C(new Set(_)),children:`全部展开`}),(0,B.jsx)(`button`,{type:`button`,onClick:()=>C(new Set),children:`全部收起`})]}),(0,B.jsx)(s,{groups:D,label:m+`导航`})]}),(0,B.jsxs)(`div`,{className:`content-shell`,children:[(0,B.jsxs)(`header`,{id:`top`,className:`modifier-library-header`,children:[(0,B.jsx)(`h1`,{children:m}),(0,B.jsxs)(`p`,{children:[u===`locations`?`每个地点单独分栏，列出其介绍、可能配置的兴趣点、挑战与任务。`:`每个兴趣点单独分栏，列出其出现地点、功能、挑战与任务。`,`先列游戏本体，再按 Mod 分栏；神祇限定内容见对应神祇页面。`]})]}),(0,B.jsxs)(`section`,{className:`section location-guide`,id:`place-guide`,children:[(0,B.jsx)(`h2`,{children:`阅读说明`}),(0,B.jsx)(o,{config:p,items:g,openEntries:S,onToggle:T})]}),h.map(e=>(0,B.jsxs)(`div`,{className:`location-collection`,id:e.id,children:[(0,B.jsxs)(`header`,{className:`location-collection-heading`,children:[(0,B.jsx)(`p`,{className:`section-index`,children:[`places-base`,`places-cordyceps`].includes(e.id)?`游戏本体`:`Mod`}),(0,B.jsx)(`h2`,{children:e.title})]}),e.articles.map(e=>(0,B.jsxs)(`section`,{className:`section location-article`,id:e.id,children:[(0,B.jsxs)(`header`,{className:`location-article-heading`,children:[e.image&&(0,B.jsx)(`img`,{src:e.image,alt:``,loading:`lazy`}),(0,B.jsxs)(`div`,{children:[(0,B.jsx)(`h3`,{children:e.name}),e.unplaced&&(0,B.jsx)(`p`,{children:`未找到正常生成路径`})]})]}),e.blocks.map((e,t)=>(0,B.jsxs)(`div`,{className:`location-article-block`+([`地点`,`兴趣点`,`条件性行动入口`].includes(e.title)?` place-overview`:``),children:[(0,B.jsx)(`h4`,{children:[`地点`,`兴趣点`].includes(e.title)?`介绍`:e.title}),(0,B.jsx)(o,{config:p,items:e.items,openEntries:S,onToggle:T})]},t))]},e.id))]},e.id)),(0,B.jsx)(`footer`,{className:`archive-footer`,children:(0,B.jsx)(`a`,{className:`sidebar-index-link`,href:`./`,children:`← 返回索引页`})})]})]})}export{V as default};