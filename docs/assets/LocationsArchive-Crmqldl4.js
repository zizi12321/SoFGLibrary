import{_ as e,a as t,d as n,f as r,g as i,m as a,p as o,r as s,s as c,t as l,v as u,y as d}from"./index-Y2irRJdB.js";var f=d(),p=[{id:`place-model`,name:`地点、兴趣点与行动入口`,text:`地点是地图上的聚居地或野外地点载体；兴趣点是附加在地点内的具体设施。
...
<CrossReference name="`+s.name+`" href="#entry-`+n+`" />`+r})}return{...e,sections:t}}),x=b.map(e=>({...e,description:e.id===`places-base`?`人类、精灵、矮人、Orc、Deep Ones 与奇观相关的通用地点和兴趣点。`:e.description,sections:e.sections.map(e=>({...e,items:e.items.filter(e=>!g[e.id]&&e.id!==`place-hook-kalastrophe-defile`)})).filter(e=>e.items.length)})).filter(e=>e.sections.length),S=b.flatMap(e=>e.sections),C=S.flatMap(e=>e.items),w=new Map(C.map(e=>[e.id,e])),T=new Map(S.flatMap(e=>e.items.map(t=>[t.id,e.title]))),E=e=>e.id?.startsWith(`location-`),D=e=>e.id?.startsWith(`location-sub-`)?`points-of-interest`:`locations`,O=e=>[...e.matchAll(/href="#entry-([^"]+)"/g)].map(e=>e[1]),k=e=>[...new Set(e.flatMap(e=>O(e.text)).filter(e=>/^place-(?:task|ruler)-/.test(e)&&!g[e]))],A=e=>{let t=e.text.split(`

`).find(e=>e.startsWith(`对应地点 / 兴趣点`));return t?O(t).filter(e=>w.has(e)&&E(w.get(e))):[]},j=e=>e.image??A(e).map(e=>w.get(e)?.image).find(Boolean),M=/^(?:共同行动|英雄任务|挑战|宗教任务|自主单位任务|统治者行动)：/,N=e=>e.split(`

`).filter(e=>!M.test(e)).join(`

`),P=e=>{let t=new Map;for(let n of e){let e=w.get(n);if(!e)continue;let r=T.get(n);t.set(r,[...t.get(r)??[],e])}return[...t].map(([e,t])=>({title:e,items:t}))};function F(e){return x.map(t=>{let n=t.sections.flatMap(e=>e.items).filter(t=>E(t)&&D(t)===e),r=t.sections.filter(e=>e.id.endsWith(`-extensions`)).flatMap(e=>e.items),i=new Set,a=n.map(n=>{let a=r.filter(e=>A(e).includes(n.id));a.forEach(e=>i.add(e.id));let o={...n,text:N(n.text)};return{id:`place-article-`+n.id,name:n.name,image:n.image,unplaced:t.sections.some(e=>e.id.endsWith(`-unplaced`)&&e.items.includes(n)),blocks:[{title:e===`locations`?`地点`:`兴趣点`,items:[o]},...a.length?[{title:`条件性行动入口`,items:a.map(e=>({...e,image:j(e),text:N(e.text)}))}]:[],...P(k([n,...a]))]}}),o=[...new Set(r.filter(e=>!i.has(e.id)).flatMap(A))].filter(t=>D(w.get(t))===e&&!n.some(e=>e.id===t));for(let e of o){let n=w.get(e),o=r.filter(t=>A(t).includes(e));o.forEach(e=>i.add(e.id)),a.push({id:t.id+`-extension-`+e,name:n.name+` · 扩展`,image:n.image,blocks:[{title:`条件性行动入口`,items:o.map(e=>({...e,image:j(e),text:N(e.text)}))},...P(k(o))]})}if(e===`locations`)for(let e of r.filter(e=>!A(e).length))a.push({id:`place-article-`+e.id,name:e.name,image:j(e),blocks:[{title:`条件性行动入口`,items:[{...e,image:j(e),text:N(e.text)}]},...P(k([e]))]});return{id:t.id,title:t.title,description:t.description,articles:a}}).filter(e=>e.articles.length)}var I=new Map,L=new Map([[`locations`,F(`locations`)],[`points-of-interest`,F(`points-of-interest`)]]),R=new Set([...L.values()].flatMap(e=>e.flatMap(e=>e.articles.flatMap(e=>e.blocks.flatMap(e=>e.items.map(e=>e.id))))));for(let e of x){let t=e.sections.flatMap(e=>e.items).filter(e=>e.id?.startsWith(`place-task-`)&&!R.has(e.id));if(!t.length)continue;let n=L.get(`locations`),r=n.find(t=>t.id===e.id);r||(r={id:e.id,title:e.title,description:e.description,articles:[]},n.push(r)),r.articles.push({id:e.id+`-additional-actions`,name:`其他地点行动`,blocks:P(t.map(e=>e.id))})}for(let e of L.values())e.sort((e,t)=>x.findIndex(t=>t.id===e.id)-x.findIndex(e=>e.id===t.id));var z=new Map;for(let[e,t]of L){let n=t.find(e=>e.id===`places-base`);if(!n)continue;let r={},i=[];for(let t of h[e]){let e=n.articles.filter(e=>t.hosts.includes(e.id.replace(`place-article-`,``))),a=new Set(t.actions),o=new Map,s=e.map(e=>e.name).join(`、`),c=`<CrossReference name="`+t.name+`" href="#`+t.id+`" meta="共用行动" text="适用范围：`+s+`。仍需满足各行动的执行条件。" />`;for(let t of e){for(let e of t.blocks)e.items=e.items.filter(n=>{if(!a.has(n.id))return!0;let i=o.get(e.title)??{title:e.title,items:[]};return i.items.some(e=>e.id===n.id)||i.items.push(n),o.set(e.title,i),r[`entry-`+t.id+`--`+n.id]=n.id,!1});t.blocks=t.blocks.filter(e=>e.items.length);let e=t.blocks.find(e=>[`地点`,`兴趣点`].includes(e.title))?.items[0];e&&(e.text+=`

可以执行：`+c+`。`)}o.size&&i.push({id:t.id,name:t.name,description:`适用范围：`+e.map(e=>{let t=e.blocks.find(e=>[`地点`,`兴趣点`].includes(e.title)).items[0];return`<CrossReference name="`+e.name+`" href="#entry-`+t.id+`" />`}).join(`、`)+`。`,blocks:[...o.values()]})}n.articles.unshift(...i),z.set(e,Object.fromEntries(Object.entries(r).map(([e,t])=>[e,`entry-`+t])))}var B=new Map;for(let[e,t]of L){let n=new Set;for(let r of t)for(let t of r.articles)for(let r of t.blocks)r.items=r.items.map(r=>{let i=r.id,a=n.has(i)?t.id+`--`+i:i;n.add(i);let o=B.get(i)??new Map;return o.has(e)||o.set(e,a),B.set(i,o),{...r,id:a}})}var V=e=>e.replace(/<CrossReference name="([^"]+)"[^>]*\/>/g,`$1`).replaceAll(`"`,`“`);for(let[e,t]of L){let n=(t,n=new Map)=>t.replace(/<CrossReference name="([^"]+)" href="#entry-([^"]+)"\s*\/>/g,(t,r,i)=>{if(y.has(i))return r;let a=n.get(i)??B.get(i)?.get(e);if(a)return`<CrossReference name="`+r+`" href="#entry-`+a+`" />`;let o=[...B.get(i)?.entries()??[]][0],s=g[i],c=w.get(i);if(!o&&!s||!c)return t;let l=V([c.text,c.location&&`执行地点：`+c.location,c.meta,c.statLine,c.limit&&`执行条件：`+c.limit].filter(Boolean).join(`
`)),u=E(c)?D(c)===`locations`?`地点`:`兴趣点`:T.get(i)??``;return`<CrossReference name="`+r+`" href="`+(s?.href??`?page=`+o[0]+`#entry-`+o[1])+`" meta="`+u+`" text="`+l+`"`+(c.image?` image="`+c.image+`"`:``)+` />`}),r=(e,t)=>({...e,text:n(e.text,t),location:e.location?n(e.location,t):void 0,limit:e.limit?n(e.limit,t):void 0});for(let e of t)for(let t of e.articles){t.description&&=n(t.description);let e=new Map(t.blocks.flatMap(e=>e.items.map(e=>[e.id.split(`--`).at(-1),e.id])));for(let n of t.blocks)n.items=n.items.map(t=>r(t,e))}let i=p.map(e=>r({...e,text:e.id===`place-model`?`地点是地图上的聚居地或野外地点载体；兴趣点是附加在地点内的具体设施。
每个条目分别列出自身提供的挑战与任务；地点修正、单位、神祇与 Mod 追加的行动另注明条件。实际能否执行还取决于执行者、渗透、资源、信仰及世界状态。`:e.text})),a=[{id:`place-guide`,title:`阅读说明`,items:i},...t.flatMap(e=>e.articles.flatMap(e=>e.blocks.map((t,n)=>({id:e.id+`-block-`+n,title:t.title,items:t.items}))))],o=Object.fromEntries(Object.entries(g).map(([e,t])=>[`entry-`+e,t.href]));for(let[t,n]of B)if(!n.has(e)){let[e,r]=[...n][0];o[`entry-`+t]=`?page=`+e+`#entry-`+r}I.set(e,{kind:e,title:e===`locations`?`地点`:`兴趣点`,sources:t,intro:i,redirects:o,aliases:z.get(e)??{},config:{id:e,assetDir:`locations`,autoLink:!1,powers:[],sections:a}})}function H(e){return I.get(e)}var U=u();function W({onReturn:u,kind:d=`locations`}){let p=H(d),{config:m,title:h,sources:g,intro:_}=p,v=m.sections.flatMap(e=>e.items.map(e=>`entry-`+e.id)),{sidebarHidden:y,setSidebarHidden:b,closeMobileSidebar:x,onSidebarClick:S}=e(),[C,w]=(0,f.useState)(new Set),T=e=>w(t=>new Set(t).add(e)),E=e=>w(t=>{let n=new Set(t);return n.has(e)?n.delete(e):n.add(e),n}),D=c(T,p.aliases);(0,f.useEffect)(()=>{let e=()=>{let e=p.redirects[window.location.hash.slice(1)];e&&window.location.replace(e)};return e(),window.addEventListener(`hashchange`,e),()=>window.removeEventListener(`hashchange`,e)},[p]);let O=[{name:`阅读说明`,entries:[{href:`#place-guide`,label:h}]},...g.map(e=>({name:e.title,entries:e.articles.map(e=>({href:`#`+e.id,label:e.name}))}))];return(0,U.jsxs)(`main`,{className:`site-shell base-modifiers-theme locations-theme`+(y?` sidebar-hidden`:``),onClick:D,children:[(0,U.jsx)(i,{hidden:y,onClose:x}),(0,U.jsxs)(`aside`,{className:`sidebar`,onClick:S,children:[(0,U.jsxs)(`div`,{className:`sidebar-head`,children:[(0,U.jsxs)(`div`,{className:`sidebar-brand`,children:[(0,U.jsx)(`span`,{className:`brand-mark`,children:d===`locations`?(0,U.jsx)(o,{size:17}):(0,U.jsx)(a,{size:17})}),(0,U.jsxs)(`span`,{children:[(0,U.jsx)(`b`,{children:h}),(0,U.jsx)(`small`,{children:`其他信息`})]})]}),(0,U.jsx)(`button`,{className:`sidebar-toggle`,type:`button`,"aria-expanded":!y,"aria-label":y?`展开侧边栏`:`收起侧边栏`,onClick:()=>b(e=>!e),children:y?(0,U.jsx)(n,{size:17}):(0,U.jsx)(r,{size:17})})]}),(0,U.jsx)(`a`,{className:`sidebar-index-link`,href:`./`,children:`← 返回索引页`}),(0,U.jsxs)(`div`,{className:`sidebar-bulk`,children:[(0,U.jsx)(`button`,{type:`button`,onClick:()=>w(new Set(v)),children:`全部展开`}),(0,U.jsx)(`button`,{type:`button`,onClick:()=>w(new Set),children:`全部收起`})]}),(0,U.jsx)(l,{groups:O,label:h+`导航`})]}),(0,U.jsxs)(`div`,{className:`content-shell`,children:[(0,U.jsxs)(`header`,{id:`top`,className:`modifier-library-header`,children:[(0,U.jsx)(`h1`,{children:h}),(0,U.jsxs)(`p`,{children:[d===`locations`?`每个地点单独分栏，列出其介绍、可能配置的兴趣点、挑战与任务。`:`每个兴趣点单独分栏，列出其出现地点、功能、挑战与任务。`,`先列游戏本体，再按 Mod 分栏；神祇限定内容见对应神祇页面。`]})]}),(0,U.jsxs)(`section`,{className:`section location-guide`,id:`place-guide`,children:[(0,U.jsx)(`h2`,{children:`阅读说明`}),(0,U.jsx)(s,{config:m,items:_,openEntries:C,onToggle:E})]}),g.map(e=>(0,U.jsxs)(`div`,{className:`location-collection`,id:e.id,children:[(0,U.jsxs)(`header`,{className:`location-collection-heading`,children:[(0,U.jsx)(`p`,{className:`section-index`,children:[`places-base`,`places-cordyceps`].includes(e.id)?`游戏本体`:`Mod`}),(0,U.jsx)(`h2`,{children:e.title})]}),e.articles.map(e=>(0,U.jsxs)(`section`,{className:`section location-article`,id:e.id,children:[(0,U.jsxs)(`header`,{className:`location-article-heading`,children:[e.image&&(0,U.jsx)(`img`,{src:e.image,alt:``,loading:`lazy`}),(0,U.jsxs)(`div`,{children:[(0,U.jsx)(`h3`,{children:e.name}),e.unplaced&&(0,U.jsx)(`p`,{children:`未找到正常生成路径`})]})]}),e.description&&(0,U.jsx)(`p`,{className:`location-common-scope`,children:(0,U.jsx)(t,{config:m,text:e.description})}),e.blocks.map((e,t)=>(0,U.jsxs)(`div`,{className:`location-article-block`+([`地点`,`兴趣点`,`条件性行动入口`].includes(e.title)?` place-overview`:``),children:[(0,U.jsx)(`h4`,{children:[`地点`,`兴趣点`].includes(e.title)?`介绍`:e.title}),(0,U.jsx)(s,{config:m,items:e.items,openEntries:C,onToggle:E})]},t))]},e.id))]},e.id)),(0,U.jsx)(`footer`,{className:`archive-footer`,children:(0,U.jsx)(`a`,{className:`sidebar-index-link`,href:`./`,children:`← 返回索引页`})})]})]})}export{W as default};